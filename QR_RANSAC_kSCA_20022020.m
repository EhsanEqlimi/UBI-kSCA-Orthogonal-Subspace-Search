...
T= c*30; %1000; % The Number of data poins a.k.a the number of samples (time points)
Sigma=1e-4; % Parameter controls the standard deviation of normal noise over the
...
Th_RansacMixing=1e-3;% RANSAC Threshold in Distance Function for mixing matrix identifiation
...
MaxIterEst=1e6;% log(1-p)/log(1-(1-e)^k); %1e6;%log(1-p)/log(1-(1-e)^k);
...
if ~isempty(length(find(Clusters==0))) && DualMode
