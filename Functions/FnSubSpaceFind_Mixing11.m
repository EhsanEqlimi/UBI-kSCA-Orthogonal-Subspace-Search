function [QRSubspaceInds,Clusters,SubSpaces,ComplementOrthofSubSpaces,ConnMat,Ahat]=FnSubSpaceFind_Mixing11(X,ThBBC,Thr,k,c,SubspaceInds,DistFunc,DegFunc,FitFunc,n,A)
X=squeeze(X);
% if numel(size(X))>=3
...
AllPNV=[];
if numel(size(X))>=3
   Selected1=reshape(X, [size(X,1),size(X,2)*size(X,3)]);
else
    Selected1=X;
end
 SelectedIndices=1:size(Selected1,numel(size(Selected1)));
       
while size(SavedCOS,2)<n && MaxN<1e4
...
%             [PNV, Inliers] = ransacV0(Selected, FitFunc, DistFunc, DegFunc, m-1, Thr);
            [PNV, Inliers] = ransac(Selected1, FitFunc, DistFunc, DegFunc, m-1 ,Thr,MaxIterEst,p);
            AllPNV=[AllPNV PNV];
...
%         end        %         [OrthSub,MinSV]=FnSubspaceCalcofInleiersV3(XSel,Inliers,k);
        if ~isempty(Inliers)
            %Inliers=Inliers(randperm(k));
                E=E+1;
           Inliers=SelectedIndices;
            [Sub,OrthSub]=FnSubspaceCalcofInleiers(Selected1,Inliers(1:20),m-1);
...
SelectedIndices=randperm(40);
% SelectedIndices=randperm(size(Selected1,numel(size(Selected1)))-RP(1));
%  SelectedIndices=1:size(Selected1,numel(size(Selected1))-RP(1));
