function [QRSubspaceInds,Clusters,SubSpaces,ComplementOrthofSubSpaces,ConnMat,Ahat]=FnSubSpaceFind_Mixing5New22(X,ThBBC,Thr,k,c,SubspaceInds,DistFunc,DegFunc,FitFunc,n,A)
X=squeeze(X);
% if numel(size(X))>=3
...
m=size(X,1);
Level=m-1;
DegFunc= @(x) FnDegenerateEval(x,Level);% For mixing ransac

while size(SavedCOS,2)<n && MaxN<2000
%     if MaxN<2000
%         Level=m-1;
%         
%     else
...
%     DegFunc= @(x) FnDegenerateEval(x,Level);% For mixing ransac
    ...
    gg=randperm(size(Selected,2));
    gg=gg(1:k);
    [PNV, Inliers] = ransac(Selected(:,gg), FitFunc, DistFunc, DegFunc, Level ,Thr,MaxIterEst,p);
    AllPNV=[AllPNV PNV];
...
%         E=E+1;
%           ff=randperm(length(Inliers));
%         [Sub,OrthSub]=FnSubspaceCalcofInleiers(Selected,ff(1:m),m-1);
        [Sub,OrthSub]=FnSubspaceCalcofInleiers2(Selected(:,gg),Inliers,m-1,1e-5);

%         [Sub,OrthSub]=FnSubspaceCalcofInleiers2(Selected,ff(1:m),m-1,1e-4);
