function [QRSubspaceInds,Clusters,SubSpaces,ComplementOrthofSubSpaces,ConnMat,Ahat]=FnSubSpaceFind_Mixing5New(X,ThBBC,Thr,k,c,SubspaceInds,DistFunc,DegFunc,FitFunc,n,A)
X=squeeze(X);
% if numel(size(X))>=3
...
if numel(size(X))>=3
    XResh=reshape(X, [size(X,1),size(X,2)*size(X,3)]);
else
    XResh=X;
end

%    Selected=reshape(X, [size(Selected1,1),size(Selected1,2)*size(Selected1,3)]);
SelectedIndices=1:size(XResh,2);

while size(SavedCOS,2)<n && MaxN<2000
    Selected=XResh(:,SelectedIndices);
    %     if MaxN<2000
    %         Level=m-1;
    %
    %     else
    ...
    [PNV, Inliers] = ransac(Selected, FitFunc, DistFunc, DegFunc, Level ,Thr,MaxIterEst,p);
    AllPNV=[AllPNV PNV];
