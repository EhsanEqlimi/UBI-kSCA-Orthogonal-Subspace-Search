function [QRSubspaceInds,Clusters,SubSpaces,ComplementOrthofSubSpaces,ConnMat]=FnSubSpaceFindV3_Distance(X,Thr,k,c,SubspaceInds,MaxIterEst,p)
%% RANSAC Functions:
...
for i=1:c
   disp(['Subspace#' num2str(i) '/' num2str(c)]) ;
    ...
    [PNV, Inliers] = ransac_distance(X(:,SelectedIndices), FitFunc, DistFunc, DegFunc, k, Thr,MaxIterEst,p);
