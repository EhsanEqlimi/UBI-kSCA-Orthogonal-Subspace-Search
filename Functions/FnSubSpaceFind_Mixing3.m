function [QRSubspaceInds,Clusters,SubSpaces,ComplementOrthofSubSpaces,ConnMat,Ahat]=FnSubSpaceFind_Mixing3(X,Thr,k,c,SubspaceInds,DistFunc,DegFunc,FitFunc,n,A)
X=squeeze(X);
if numel(size(X))>=3
...
for i=1:c
    ...
    if length(SelectedIndices)>k
        [PNV, Inliers] = ransacV0(X(:,SelectedIndices), FitFunc, DistFunc, DegFunc, g, Thr);
