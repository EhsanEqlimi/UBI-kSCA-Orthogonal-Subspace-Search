function [QRSubspaceInds,Clusters,SubSpaces,ComplementOrthofSubSpaces,ConnMat,Ahat]=FnSubSpaceFind_Mixing5_V2(X,ThBBC,Thr,k,c,SubspaceInds,DistFunc,DegFunc,FitFunc,n,A)
X=squeeze(X);
% if numel(size(X))>=3
...
%             [PNV, Inliers] = ransacV0(Selected, FitFunc, DistFunc, DegFunc, m-1, Thr);
            [PNV, Inliers] = ransac(Selected, FitFunc, DistFunc, DegFunc, m+2 ,Thr,MaxIterEst,p);
            AllPNV=[AllPNV PNV];
