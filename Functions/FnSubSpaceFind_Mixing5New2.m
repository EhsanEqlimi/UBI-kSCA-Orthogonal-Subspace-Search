function [QRSubspaceInds,Clusters,SubSpaces,ComplementOrthofSubSpaces,ConnMat,Ahat]=FnSubSpaceFind_Mixing5New2(X,ThBBC,Thr,k,c,SubspaceInds,DistFunc,DegFunc,FitFunc,n,A)
X=squeeze(X);
% if numel(size(X))>=3
...
MaxN=0;
AllPNV=[];
Selected=reshape(X, [size(X,1),size(X,2)*size(X,3)]);
SelectedIndices=nchoosek(1:size(Selected,2),size(Selected,1));

while size(SavedCOS,2)<n && MaxN<3e3
...
%             [PNV, Inliers] = ransacV0(Selected, FitFunc, DistFunc, DegFunc, m-1, Thr);
            [PNV, Inliers] = ransac(Selected, FitFunc, DistFunc, DegFunc, m-1 ,Thr,MaxIterEst,p);
            AllPNV=[AllPNV PNV];
