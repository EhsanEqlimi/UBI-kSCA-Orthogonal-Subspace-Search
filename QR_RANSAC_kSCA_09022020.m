...
T= 1000; % The Number of data poins a.k.a the number of samples (time points)
Sigma=0;%1e-17; % Parameter controls the standard deviation of normal noise over the
...
Th_RansacSubspace=1e-9;% RANSAC Threshold in Distance Function for subspace estimation
Th3=1e-8;
Sigma1=0.02;
Th_RansacMixing=1e-8;% RANSAC Threshold in Distance Function for mixing matrix identifiation
Iter_MixingId=50*c; 
p=0.99; % Desired probability that we get a good sample in ransac
e=0.1;  % Probability that a point is an outlier
MaxIterEst=log(1-p)/log(1-(1-e)^c);
%% RANSAC Functions:
...
g=nchoosek(n-1,k-1);
DegFunc2= @(x) FnDegenerateEval(x,m);
...
[QRSubspaceInds,Clusters,SubSpaces,ComplementOrthofSubSpaces,ConnMat]=FnSubSpaceFindV0(X,Th_RansacSubspace,k,c,SubspaceInds,DistFunc,DegFunc,FitFunc);
