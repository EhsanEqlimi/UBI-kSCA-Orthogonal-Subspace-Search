...
Sigma_Off=[1e-9]; % Parameter controls the standard deviation of normal noise over the active lements
...
BAS_Set=[];NMSE_Set=[];Norm2Error_Set=[];Sub_Error_Set=[];
...
%                 X=FnColNormalizer(X);
                ...
                ET(CT)=toc;
                %                 [MixingIdentificatioError, MixingVectorerror,NMSE,NMSSum,AhatNew,Norm2Error] = FnMixingIdentificationError(A,Ahat);
                %
...
% end
Count=0;
for i=1:length(Ahat_Set)
    nhat(i)=size(Ahat_Set{i},2);
    if nhat(i)==n
        Count=Count+1;
        [MixingIdentificatioError(Count), MixingVectorerror,NMSE,NMSSum(Count),AhatNew,Norm2Error(Count)] = FnMixingIdentificationError(A_Set{i},Ahat_Set{i});
    end
end
nhatMean=mean(nhat)
mean(MixingIdentificatioError)
mean(NMSSum)
mean(Norm2Error)
